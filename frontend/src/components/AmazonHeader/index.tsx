  ...
  return (
    <header style={{ position: "sticky", top: 0, zIndex: 50 }}>
            ...
            onClick={() => {
              const userId =
                typeof localStorage !== "undefined"
                  ? JSON.parse(localStorage.getItem("amazon_now_user") || "{}")
                      .user_id || "demo_user"
                  : "demo_user";
              fetch(
                `${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000"}/api/v1/calendar/auth-url?user_id=${userId}`,
              )
                .then((r) => r.json())
                .then((data) => {
                  if (data.auth_url) {
      ...
      >
        ...
        <span style={{ fontSize: 11, color: "#CCCCCC", fontWeight: 700 }}>
          ▾
        </span>
      </div>
      ...
      >
        <div
          style={{
            ...
            gap: 8,
            border: "1.5px solid",
            borderColor: focused ? "#FF9900" : "#CCC",
            borderRadius: 8,
            overflow: "hidden",
            background: "white",
          }}
