...
export function AmazonHeader({ cart, onCartClick, onProductSelect }: Props) {
  ...
  const [calMode, setCalMode] = useState<"oauth" | "service_account" | "mock">(
    "mock",
  );
  ...
  const getUserId = () => {
    try {
      return (
        JSON.parse(localStorage.getItem("amazon_now_user") || "{}").user_id ||
        "demo_user"
      );
    } catch {
      return "demo_user";
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/v1/calendar/status?user_id=${uid}`)
      .then((r) => r.json())
      .then((d) => {
        setCalLinked(d.linked);
        setCalMode(d.mode ?? "mock");
      })
      .catch(() => {});
  }, []);
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/v1/calendar/events?user_id=${uid}`,
      );
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (
        calPanelRef.current &&
        !calPanelRef.current.contains(e.target as Node)
      ) {
        setCalOpen(false);
      }
    };
  ...
  return (
    <header style={{ position: "sticky", top: 0, zIndex: 50 }}>
      ...
      >
        ...
        >
          ...
          >
            ...
            <span style={{ fontSize: 11, fontWeight: 600, color: "#0F1111" }}>
              Calendar
            </span>
            {calLinked && (
              <span
          ...
          {calOpen && (
            ...
            >
              ...
              >
                ...
                <button
                  onClick={
                    calMode === "service_account"
                      ? undefined
                      : handleLinkCalendar
                  }
                  style={{
                    background:
                      calLinked || calMode === "service_account"
                    ...
                    fontWeight: 700,
                    cursor:
                      calMode === "service_account" ? "default" : "pointer",
                    color:
                      calLinked || calMode === "service_account"
                        ? "white"
                        : "#0F1111",
              ...
              <div style={{ maxHeight: 260, overflowY: "auto" }}>
                {calLoading ? (
                  ...
                  <div style={{ padding: "20px", textAlign: "center" }}>
                    <p
                      style={{ margin: "0 0 4px", fontSize: 13, color: "#555" }}
                    >
                      No events today
                    </p>
      ...
      >
        ...
        >
          <span style={{ fontSize: 10, fontWeight: 800, color: "#0F1111" }}>
            ⚡8
          </span>
          <span style={{ fontSize: 10, color: "#0F1111", fontWeight: 600 }}>
            mins
          </span>
        </div>
        ...
        >
          Deliver to{" "}
          <strong>
            Flat no-301, Geetanjali Aristocracy, Whitefield, Bengaluru
          </strong>
        </span>
        <span style={{ fontSize: 10, color: "#00695C", fontWeight: 700 }}>
          ▾
        </span>
      </div>
      ...
      >
        <div
          style={{
            ...
            gap: 8,
            border: "1.5px solid",
            borderColor: focused ? "#00695C" : "#CCC",
            borderRadius: 8,
            overflow: "hidden",
            background: "white",
          }}
              ...
              results.map((product, i) => (
                <div
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    borderBottom:
                      i === results.length - 1 ? "none" : "1px solid #F5F5F5",
                    background: "white",
                  }}
                  onMouseEnter={(e) =>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p
                      style={{ margin: "2px 0 0", fontSize: 10, color: "#888" }}
                    >
                      {product.unit} · ⚡{product.eta_min} min
                    </p>
                  </div>
                  <p
                    style={{
