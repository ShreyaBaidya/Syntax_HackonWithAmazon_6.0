...
export function SmartFridgeWidget({ onAddAll }: { onAddAll: (items: any[]) => void }) {
  ...
  return (
    <div style={{ margin: '8px 10px 0', background: 'white', borderRadius: 10, overflow: 'hidden', border: '1px solid #E0F7FA' }}>
      <div style={{
        padding: '12px 14px',
        ...
        <div style={{ flex: 1 }}>
          <p style={{ margin: 0, fontWeight: 800, fontSize: 13, color: '#006064' }}>
            Smart Fridge Alert
          </p>
      ...
      <div style={{ padding: '8px 14px', background: '#FAFAFA' }}>
        {status.auto_cart.map((item, idx) => (
          <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 4 }}>
            <span style={{ fontWeight: 600, color: '#333' }}>{item.name} <span style={{color: '#888'}}>x{item.suggested_qty}</span></span>
