      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-md bg-white z-[70] shadow-2xl flex flex-col animate-slide-in-right">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div className="flex items-center gap-2">
            <ShoppingCart className="h-5 w-5 text-amazon-orange" />
            <h2 className="text-lg font-bold text-gray-800">Your Cart</h2>
            <span className="bg-amazon-orange text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
              {itemCount}
            </span>
          </div>
          <button
                  ...
                  <div className="flex flex-col items-end gap-2">
                    {item.estimated_price && (
                      <p className="font-semibold text-gray-800 text-sm">
                        {formatPrice(item.estimated_price * item.count)}
                      </p>
                    )}
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        className="p-1 rounded-md hover:bg-red-50 text-gray-400 hover:text-red-500 ml-1"
                      >
        ...
        {items.length > 0 && (
          <div className="border-t px-6 py-4 space-y-3">
            ...
            <button className="w-full bg-amazon-orange hover:bg-amazon-orange-dark text-white font-semibold py-3 rounded-xl">
              Proceed to Checkout
            </button>
            <button
              onClick={clearCart}
