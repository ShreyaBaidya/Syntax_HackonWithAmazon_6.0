...
export default function CartPage() {
  ...
  const [step, setStep] = useState<'loading' | 'name' | 'ready' | 'error'>('loading');
  ...
  if (step === 'error') {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#F7F7F7', padding: 24 }}>
        ...
        <a href="/" style={{ marginTop: 16, background: '#FFD814', color: '#0F1111', padding: '10px 24px', borderRadius: 8, fontWeight: 700, fontSize: 14, textDecoration: 'none' }}>
          Go Home
        </a>
      </div>
    );
  ...
  if (step === 'name') {
    return (
      <div style={{ minHeight: '100vh', background: '#F7F7F7', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{ background: 'white', borderRadius: 12, padding: 28, maxWidth: 360, width: '100%', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
          <div style={{ textAlign: 'center', marginBottom: 20 }}>
            <div style={{ fontSize: 48, marginBottom: 8 }}>🛒</div>
            <h2 style={{ margin: '0 0 4px', fontSize: 20, fontWeight: 800 }}>Join Shared Cart</h2>
            <p style={{ margin: 0, color: '#888', fontSize: 13 }}>
              Cart <strong>{cartId}</strong> · {cart?.participants.length ?? 0} participant{(cart?.participants.length ?? 0) !== 1 ? 's' : ''} online
            </p>
          </div>
          ...
          <input
            ...
            autoFocus
          />
          {error && <p style={{ color: '#CC0C39', fontSize: 12, margin: '0 0 10px' }}>{error}</p>}
          <button
            onClick={handleJoin}
            style={{
              width: '100%', background: '#FFD814', color: '#0F1111',
