...
function findBestCoupon(
  ...
  return eligible.reduce((best, c) =>
    computeCouponSavings(c, subtotal) > computeCouponSavings(best, subtotal)
      ? c
      : best,
  );
}
...
const PAYMENT_METHODS: {
  ...
  { id: "upi", icon: "📱", label: "UPI", sub: "GPay · PhonePe · Paytm" },
  {
    ...
    icon: "🛡️",
    label: "Amazon Pay",
    sub: "Balance: ₹1,250.00",
  },
  {
    id: "card",
    icon: "💳",
    label: "Credit / Debit Card",
    sub: "Visa ending ·· 4321",
  },
  {
    id: "cod",
    icon: "💵",
...
export default function CartPage() {
  ...
  const coupon =
    pinnedCoupon && subtotal >= pinnedCoupon.min_subtotal
      ? pinnedCoupon
      : bestCoupon;
    ...
    try {
      ...
      if (sub > 0) {
        fetchCoupons(sub)
          ...
          .catch(() => {
            /* backend unavailable — no coupons shown */
          });
      }
    } catch {
      /* ignore */
  ...
  const deliveryFee =
    coupon?.type === "delivery" ? 0 : subtotal > 0 ? DELIVERY_FEE : 0;
  const total = Math.max(0, subtotal - couponSavings + deliveryFee);
  const totalSavings =
    couponSavings + (coupon?.type === "delivery" ? DELIVERY_FEE : 0);
  const itemCount = items.reduce((s, i) => s + i.quantity, 0);
  const maxEta = items.length
    ? Math.max(...items.map((i) => i.product.eta_min))
    : 28;
          ...
          {totalSavings > 0 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 mb-4">
              <p className="text-sm font-semibold text-yellow-800">
                🎉 You saved ₹{Math.round(totalSavings)} with {coupon?.code}!
              </p>
            </div>
          )}
      ...
      <div className="min-h-screen bg-[#F0F2F2] flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm mb-6">
            Add items from the home feed or use NowSpeak.
          </p>
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F2]">
      ...
      <div className="bg-[#131921] px-4 py-3 flex items-center gap-3 sticky top-0 z-10 shadow-md">
        <button
          onClick={() => router.push("/")}
          className="text-white hover:text-[#FFD814] transition-colors p-1"
        >
        ...
        <span className="text-[#FFD814] text-xs font-semibold">
          ⚡ {maxEta} min
        </span>
      </div>
              ...
              >
                ...
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  ...
                  <div className="flex items-center bg-[#FFD814] rounded-full border border-[#F0C000] overflow-hidden">
                    ...
                    <span className="px-1 text-sm font-bold min-w-[22px] text-center">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() =>
                ...
                <button
                  onClick={
                    () =>
                      setPinnedCoupon(
                        null,
                      ) /* removes pin → reverts to auto-best */
                  }
              ...
              <div
                ...
                className={`border rounded-xl p-4 transition-all select-none ${
                  isActive
                    ...
                    : c.eligible
                      ? "border-gray-200 hover:border-[#FF9900] hover:shadow-sm cursor-pointer"
