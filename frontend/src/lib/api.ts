...
export type CouponResult = {
  code: string; label: string; description: string; type: CouponType;
  min_subtotal: number; savings: number; eligible: boolean; badge?: string | null;
  discount_amount?: number | null; discount_rate?: number | null; discount_cap?: number | null;
};
export type CouponsResponse = { best: CouponResult | null; all: CouponResult[]; };
...
// Product types
export type Product = {
  id: string; name: string; price: number; unit: string; image_url: string;
  eta_min: number; category: string; reason?: string; ingredients?: string[];
  dietary_tags?: string[]; allergen_tags?: string[];
  nutrition_summary?: { calories?: number; protein?: string; carbs?: string; fat?: string };
  is_alternative?: boolean; replaces?: string;
};
...
export type Recommendations = {
  time_context: string; now_suggestions: Product[]; reorder_nudges: Product[];
  trending: Product[]; alternatives?: Product[];
};

// Shared Cart types
export type CartItem = {
  product_id: string; name: string; price: number; unit: string;
  image_url: string; category: string; quantity: number; added_by: string[];
};
export type CartState = {
  cart_id: string; items: Record<string, CartItem>; participants: string[];
  total: number; item_count: number; created_at: string;
};
export type CartSSEEvent = {
  type: 'cart_update' | 'participant_joined' | 'participant_left' | 'checkout' | 'error';
  cart?: CartState; message?: string;
};
export type Order = {
  order_id: string; status: string; estimated_delivery: string;
  eta_minutes: number; total_amount: number;
};
export type OrderHistoryItem = {
  order_id: string; status: string; estimated_delivery: string; eta_minutes: number;
  total_amount: number; items: { product_id: string; quantity: number }[];
  delivery_address: string; created_at: string; payment_method?: string;
};
export type RefillItem = Product & {
  refill_info: {
    avg_gap_days: number; days_since_last: number; urgency: number;
    frequency: 'daily' | 'weekly' | 'biweekly' | 'monthly'; reason: string; last_purchased: string;
  };
  ai_reason?: string;
};
export type RefillGroup = { label: string; sublabel: string; items: RefillItem[]; };
export type RefillSuggestions = {
  bundle_name: string; subtitle: string; total: number; item_count: number;
  items: RefillItem[];
...
// Auth API
export type AuthUser = { token: string; user_id: string; name: string; email: string; avatar: string; };
...
export type AICartItem = {
  ...
  estimated_price?: number; substitute_available?: boolean; substitutes?: string[];
};
export type NutritionInfo = { calories: number; protein: number; carbs: number; fat: number; fiber?: number; };
export type AICartResponse = {
  intent: string; description: string; items: AICartItem[];
  total_estimated_price?: number; nutrition?: NutritionInfo; budget_tier: BudgetTier;
};
export type Occasion = { id: string; name: string; icon: string; description: string; };
...
export async function buildCartFromIntent(query: string, budgetTier: BudgetTier = 'standard', dietaryPreferences: string[] = []): Promise<AICartResponse> {
...
export async function buildCartFromURL(url: string, prompt?: string, budgetTier: BudgetTier = 'standard'): Promise<AICartResponse> {
  return _aiPost('/api/v1/cart/url', { url, prompt, budget_tier: budgetTier });
}
export async function buildCartForOccasion(occasion: string, guests = 2, budgetTier: BudgetTier = 'standard', dietaryPreferences: string[] = []): Promise<AICartResponse> {
  return _aiPost('/api/v1/cart/occasion', { occasion, guests, budget_tier: budgetTier, dietary_preferences: dietaryPreferences });
}
export async function buildCartForOutcome(goal: string, budgetTier: BudgetTier = 'standard', dietaryPreferences: string[] = []): Promise<AICartResponse> {
