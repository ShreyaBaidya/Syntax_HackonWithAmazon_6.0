...
from app.schemas.cart import (
    IntentRequest, CartResponse, URLPromptRequest,
    OccasionRequest, OutcomeRequest, BudgetTier, CartItem, ItemCategory
)
...
def _map_to_catalog(items: list[CartItem]) -> list[CartItem]:
    ...
    for item in items:
        ...
        print(f"DEBUG _map_to_catalog: query='{item.name}' results={bool(results)}", flush=True)
        if results:
            real = results[0]
...
async def build_cart_from_intent(request: IntentRequest):
    ...
    items = await product_agent.build_cart_from_intent(
        intent_data=intent_data, query=request.query,
        budget_tier=request.budget_tier, dietary_preferences=request.dietary_preferences
    )
    if not items:
...
async def get_occasions():
    return {"occasions": [
        {"id": "sick_day", "name": "Sick Day", "icon": "🤒", "description": "Feel better food & remedies"},
        {"id": "game_night", "name": "Game Night", "icon": "🎮", "description": "Snacks & drinks for gaming"},
        {"id": "movie_night", "name": "Movie Night", "icon": "🎬", "description": "Popcorn, snacks & treats"},
        {"id": "house_party", "name": "House Party", "icon": "🎉", "description": "Party food & beverages"},
        {"id": "birthday", "name": "Birthday", "icon": "🎂", "description": "Cake ingredients & party food"},
        {"id": "date_night", "name": "Date Night", "icon": "💕", "description": "Romantic dinner ingredients"},
        {"id": "office_lunch", "name": "Office Lunch", "icon": "💼", "description": "Quick & professional meals"},
        {"id": "quick_breakfast", "name": "Quick Breakfast", "icon": "☀️", "description": "Fast morning meals"},
    ]}
