...
def _resolve_product(pid: str, pattern: dict) -> dict | None:
    ...
    catalog_hit = next(
        (p for p in AMAZON_PRODUCTS if p.get("sku") == pid or p.get("id") == pid), None
    )
    if catalog_hit:
        return {
            "id": pid,
...
Apply the rules. Return JSON array."""
        ...
        cart_keywords = {
            w.lower() for name in cart_item_names for w in name.split() if len(w) > 3
        }
        ...
        for key, group in type_groups.items():
            ...
            best = max(
                group, key=lambda x: x.get("refill_info", {}).get("purchase_count", 0)
            )
            best = dict(best)
...
def get_refill_suggestions(
    ...
    suggestions.sort(
        key=lambda x: x.get("refill_info", {}).get("urgency", 0), reverse=True
    )
    ...
    return {
        ...
        "grouped": {
            "today": {
                ...
                "sublabel": "Based on your calendar events",
                "items": today,
            },
            "weekly": {
                "label": "📅 Weekly Restock",
                "sublabel": "Every 5–10 days",
                "items": weekly,
            },
            "biweekly": {
                "label": "🗓️ Bi-weekly",
                "sublabel": "Every 11–20 days",
                "items": biweekly,
            },
            "monthly": {
                "label": "📦 Monthly Pantry",
                "sublabel": "Every 3–4 weeks",
                "items": monthly,
            },
        },
    }
